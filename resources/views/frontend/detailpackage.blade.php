...
<div class="row mt-5 mb-5 ">
  ...
  <div class="col-lg-3 col-md-3 col-sm-12 dd1">
  	@foreach($services as $key => $value)
    <div class=" dd nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
      <a class=" nav-link active detail" data-id="{{$value->id}}" data-name="{{$value->name}}" data-packagename="{{$package->name}}" data-pid="{{$package->id}}" id="v-pills-home-tab" data-toggle="pill" href="#p{{$i}}" role="tab" aria-controls="v-pills-home" aria-selected="true">{{$value->name}}</a>
      
    </div>
     @endforeach
    <div  class="mt-5 pt-5" >
    	<a class="btn btn-primary back" href="{{route('package')}}">
